...
- Step 5（规则化归一与校验）：已完成（最新前缀 `step5_seq_step2_v2_rebind14_fixabcd_plus2`）
- Step 6（Gold/IAA 构建与一致性评测）：已完成（最新前缀 `step6_iter4_fixabcd_plus`，全量门禁达标）
- 交付文件：
...
5. 规则化归一与校验
   - 正则/词典/单位解析器
   - 证据对齐（span 回溯）与一致性检查
  - 当前状态：已完成（最新 `rebind14_fixabcd_plus2` 已产出并通过门禁）

6. Gold/IAA 构建与一致性评测
   - 冻结 Step5 输入版本、分母口径与样本抽样规则
   - 生成机器辅助金标（双盲两遍）并进行自动仲裁
   - 输出 IAA 指标（kappa/F1/一致率）与误差簇分析
  - 当前状态：已完成（`step6_iter4_fixabcd_plus_iaa_report.json` 显示 `all_targets_passed=true`）
